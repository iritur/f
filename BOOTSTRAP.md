...
| ~~Every page is 2 MiB and every mapping is writable. No read-only text, no 4 KiB granularity, no per-process address spaces.~~ Closed. The kernel window is 4 KiB pages with per-section permissions (E0-B19), the direct map is no-execute, and a process has an address space of its own whose upper half is a copy of the kernel's (E0-B09). All four are proved by faulting rather than asserted: `cargo xtask fault nx` and `fault wx` from ring 0, `cargo xtask user` from ring 3. | done |
| ~~`targets/x86_64-f.json` is shipped but unused — `xtask` builds against the built-in `x86_64-unknown-none`.~~ Closed at E0-D09, by deleting it. Everything it stated that the built-in does not is two codegen flags, and those are in `.cargo/config.toml` with the reason beside them. The condition that would bring a custom target back is a doc comment on `KERNEL_TARGET` in `xtask/src/main.rs`, which is where somebody would look. | done |
