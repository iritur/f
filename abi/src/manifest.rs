...
/// pay it.
///
/// **Three since RFC 0065**, and the second change to pay that same cost. A
/// schema-2 record declares no [`Record::state`], so a schema-2 component
/// publishes no state tree — and a supervisor that read that silence as *this
/// component has nothing to say about itself* would be reinventing exactly the
/// tolerance RFC 0013 was written against. The refusal is
/// `ADMISSION/NO_STATE_TREE` and it happens at the spawn; the schema bump is
/// what makes a record that could not carry the declaration unreadable rather
/// than quietly mute.
...
pub struct Record {
    ...
    /// differ in bytes nobody reads.
    ///
    /// It took the first of the three reserved bytes below, which is what a
    /// reserved byte is for and is why taking one is a schema bump.
    pub state_nodes: u8,
    /// Reserved. Must be zero — a non-zero value is refused rather than
    /// ignored, per R04.
    /// Unit: none; this is not a quantity and is not expected to become one
    ...
    ///
    /// Last in the record because it was added last and because putting it
    /// there is what kept [`Record::transfer`], [`Record::capability`] and
    /// [`Record::ring`] at the offsets `xtask::manifest` already mirrors: a
    /// field inserted before an array moves every slot in it, and a component
    /// file the writer and the reader disagree about is exactly what the three
    /// offset assertions below exist to prevent.
...
const _: () = assert!(core::mem::size_of::<Node>() == 28);
const _: () = assert!(core::mem::size_of::<Record>() == 2680);
...
const _: () = assert!(core::mem::offset_of!(Record, state) == 2232);
...
impl Record {
    ...
    pub fn read(module: &[u8]) -> Result<&Self, Refusal> {
        ...
        if record.state_nodes as usize > STATE_NODES_MAX {
            return Err(Refusal::Count);
        }
