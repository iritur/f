...
- [ ] **E0-P02** `M` **The reproduction check.** Two runs of the same `(seed, commit)` on two different runners produce a byte-identical execution trace hash.
  ...
  *needs:* E0-B01
- [ ] **E0-P03** `S` Coverage instrumentation reported per crate by `xtask coverage`, while the kernel is small enough for it to be trivial.
  *exit:* a coverage number appears in CI output and is stored with the run.
