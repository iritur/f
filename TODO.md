...
- [ ] **E0-B15** `M` Doorbell: kernel IPI path first; user-interrupt path behind a negotiated feature bit.
  **Found before the task started, and fixed ahead of it: the suppression protocol had a lost wakeup.** The two ends run Dekker's algorithm at exactly one place — the producer stores `head` then loads `flags`, the consumer stores `flags` then loads `head` — and a store followed by a load of a *different* location is the one reordering total store order permits. `Release` and `Acquire` do not forbid it; they are one-way barriers and this needs a two-way one. Both ends could look and see nothing: the producer reads the flag before its own publish is visible and rings nothing, the consumer reads the ring before that publish and sleeps. The entry is stranded. It is not a data race — every value read was legitimately written, and no sanitiser finds it — it is a hang.
  ...
  It is the first defect in `ring/tests/litmus.rs` that does **not** need the arm runner: store-load is the reordering x86-64 performs. With the fence removed, `a_sleeping_consumer_is_never_left_holding_work` reports **58 971 lost wakeups in 500 000 rounds, first at round 69** — eight in a thousand, not a rare interleaving. The harness had to change to see it at all: a `std::sync::Barrier` ends in a futex wakeup measured in microseconds, and the window is one store buffer deep, so a parked-thread barrier lines the two threads up thousands of times too loosely and reports a clean run on a broken build. A spin barrier finds it in sixty-nine rounds and runs two hundred times faster.
  ...
  *exit:* **not met.** What is done is the suppression protocol's correctness and its evidence, above. What is not: the IPI path itself, the typed doorbell — `Path` selected from the negotiated feature set, with `Polling`, `KernelIpi` and `UserInterrupt` behind one function, which `ring-scene-boot` section 03 asks for by name — and the number. Two of those three are blocked and it is worth saying on which: the user-interrupt path needs `SENDUIPI`, and QEMU's TCG backend implements no part of Intel UINTR and no `-cpu` model advertises the bit, so it can be built to the point of refusing to construct and no further (`E1-B09` owns the hardware). "Doorbells per operation under load" is a number, so it is a claim, and a count taken under TCG with `-smp 2` measures the emulator's scheduling of two vCPUs — `F_ENVIRONMENT=container` refuses it for exactly that reason, and it waits on `E0-D10` alongside 0001 and 0002.
  *needs:* E0-B12
...
- [>] **E0-P01** `M` CI pull-request gate under ten minutes: lint, test, run, claims.
  ...
  *exit:* **half met, and the other half is blocked on a claim that gates.** The green half is written and runs; the red half — "red on a pull request that regresses a gating claim" — cannot be built, because no claim gates. 0001 and 0002 are `pending` on a measurement environment that does not exist here, and 0003 is `tracked` on purpose. That is `E0-P05` and `E0-P06`, and it is a real dependency this task's `needs:` did not name.
  ...
  Marked `[>]` with `E0-P07` and `E0-P14`, all three waiting on the same thing: a green run of this gate on a pull request.
...
- [>] **E0-P02** `M` **The reproduction check.** Two runs of the same `(seed, commit)` on two different runners produce a byte-identical execution trace hash.
  ...
  *exit:* **half met, and the half that is not is the same blocker as `E0-P14`.** Locally: two boots agree on `0x90a3830cce8ad586`, and the defect build produces two different hashes — so the check works and can fail. The CI job is written — two `trace` jobs on separate runners, a `reproduction` job comparing the artefacts, plus the defect half — and cannot be observed from here.
...
- [>] **E0-P14** `S` CI runs inside the development image rather than installing tools per job.
  ...
  *exit:* met for the parts this repository can state — every job names the image, no job installs a tool — and the observation that it *works* is a green run of this gate. Kept `[>]` until there is one, for the reason `E0-B16` recorded the hard way: that wrapper had never run once, and a failure that reports success is worse than one that does not.
  *needs:* E0-B16
...
- [ ] **E0-P16** `M` RustMC on the cursor protocol: the model check the litmus tests are not.
  ...
  **The honest fraction is built, and it turned out to be the half nobody had noticed was missing.** This exit says a model checker must catch *weakening the publishing store to `Relaxed`* — and until now no such defect existed in the tree, so the criterion could not have been attempted even with a checker in hand. `mutate-relaxed-submission` now weakens both the single-entry and the batch store; CI turns it on and requires the litmus suite to go red, on the AArch64 runner where it means something. That is evidence about the stress suite and not about what RC11 permits, and it is described as exactly that wherever it appears.
...
- [>] **E0-P17** `S` Litmus stress for the completion ring, on both architectures.
  ...
  **The third test was wrong first, and the completion ring is where it had to be.** `a_hostile_client_cursor_never_panics` was written by copying the submission ring's case list, which includes `u32::MAX`. On the completion ring that is not hostile: cursors wrap, and with `head` at zero a `tail` of `u32::MAX` is the legitimate state of a ring with one completion outstanding across the wrap — `cursors_may_wrap` asserts exactly that. The two rings have their cursors' roles the other way round, which is the thing the copy silently dropped. The list is now the differences a wrap cannot explain, with the reason written beside it.
  The defect is a feature rather than a paragraph: `mutate-relaxed-completion` in `ring/Cargo.toml` weakens the posting store to `Relaxed`, and the litmus job requires the suite to fail with it — on the AArch64 runner only, because on total store order the weakening is invisible and requiring a failure there would be requiring a flake. `!` and not `continue-on-error`: what is asserted is a failure, so the step goes red when the command succeeds.
  ...
  *exit:* **the code half is met and the observation is not.** Locally: seven litmus tests pass `--release`, three of them new and driving `Poster`/`Collector`; the defect build compiles and, as documented, passes on x86-64 — which is the whole reason the file exists. "Green on both architectures" and "the defect makes them fail" are properties of workflow runs, and nothing here can run GitHub Actions. Marked `[>]` alongside `E0-P07` and `E0-P14`, which clear on the same event.
  *needs:* E0-B12
...
- [>] **E0-R01** `M` `cargo xtask release` produces the full package: source tag, claims snapshot, content-addressed QEMU image, seed corpus, baseline configurations, and a dependency manifest.
  ...
  *exit:* **half met, and the unmet half is a property of two machines.** `cargo xtask release --twice` packages the same tree twice and requires one address; it passes, and the command says in its own output why that is the weaker half — directory order, uid, path and clock are all constant within one machine. The real question needs two runners at one commit, and it needs them to build at the same absolute path: the image is a debug build, so its DWARF carries the checkout path and cargo's `-Cmetadata` derives from a package id containing it, which `--remap-path-prefix` will not remove. That is a release-workflow question, and it is why this is `[>]`.
  *needs:* E0-D08, E0-P02
