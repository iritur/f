...
- [>] **E0-P16** `M` RustMC on the cursor protocol: the model check the litmus tests are not.
  ...
  *what is not yet known, and it is one command:* whether `f-ring` compiles under `nightly-2025-08-20`. The evidence says it should — `ring` and `abi` are edition-2024 crates with no `#![feature]` gates, and edition 2024 has been stable since 1.85 — but that is an inference and this file does not get to promote inferences. `cargo +nightly-2025-08-20 build -p f-ring` settles it, and installing a second toolchain was refused in the environment this was written in. If it fails, RFC 0022 is superseded and waiting comes back with the trigger it currently lacks.
  *what remains:* the image target, and the small tests. Neither is blocked on anything outside this repository any more, which is the change this cycle made — the blocker was a question, and it has an answer.
