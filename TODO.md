...
- [>] **E2-B01** `L` Blob store: content addressing, content-defined chunking, the on-disk format.
  ...
  **The chunker half is built; the format half is not.** `blob/` (package `f-blob`) carries the gear table and a chunker that is three words, holds no bytes and hashes as it scans, with `Cuts` finishing its slice on `Drop` so that a partially consumed iterator cannot make an object's boundaries a function of the shape of the loop that read it. `abi/src/store.rs`'s superblock and object records, `blob/src/device.rs`, `blob/src/store.rs` and `blob/tests/million.rs` are `intent/0006-state/plan.md` step 6 and are not written; nothing in this tree has ever formatted a device, which is exactly the position RFC 0061 said made its reversal cheap and `E2-B02` will not.
  ...
  *exit:* not met, on both clauses. A million blobs cannot be written, read back or verified because no store, no device model and no `million` test exist yet. And *an edit near the start of a large object re-chunks only a bounded region* is `E2-P02`, which is `[>]` and red: the published bound holds on 32 of 32 pairs — an effective sample of 18, because on the other 14 the starved allowance reaches past the edited object's own end and the assertion cannot fail there — and the clause RFC 0061's confirming run requires to carry them does not.
