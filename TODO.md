...
- [>] **E0-P14** `S` CI runs inside the development image rather than installing tools per job.
  ...
  *exit:* **not met, and cannot be met from here.** The exit is a property of workflow runs, and nothing in this environment can run GitHub Actions. What remains is: publish the image once by hand — `image.yml` triggers on a change to `docker/` or the toolchain pin, and until it has run every `ci.yml` job will fail to start — then open a pull request and read the result. Marked `[>]` rather than `[x]` for the reason `E0-B16` recorded the hard way: that wrapper had never run once, and a failure that reports success is worse than one that does not.
  `:latest` rather than a digest, deliberately and with the gap named. `docker/README.md`'s *Reproducibility, honestly* section already says the image is repeatable without being reproducible in the sense `E2-P06` uses the word. Pinning CI to a digest is one edit and belongs with that decision rather than ahead of it.
  *needs:* E0-B16
