...
- [x] **E1-B02** `L` virtio-blk in user space.
  ...
  **What the title still owes, said plainly.** `user/virtio-blk/` is a component crate that inherits `unsafe_code = "forbid"` and builds into a content-addressed component file — but nothing schedules a component, so `Driver::execute` is called from the frame rather than from a component's own polling loop. Two blockers, both `E1-B08`'s: no scheduler, and no safe channel adoption. RFC 0033 states the reversal as a measurement anybody can take — grep for `Driver::execute` and see which crate calls it.
  *needs:* E1-B01
...
- [>] **E1-B05** `L` Component supervisor: spawn from a manifest, restart policy, control ring delivery.
  ...
  *exit:* **not met, and it never could have been by this task alone.** The exit is `E1-P06 passes`, and `E1-P06` is a separate open task that kills drivers *under sustained load* — which needs a scheduler, which is `E1-B08`. This is the third exit on this list written as another task's observation, after `E0-B12` and `E1-B01`. What is owed here in its own right: the four opcodes, the seven notice kinds, uniform teardown, and the restart policy running where RFC 0008 puts it.
...
- [>] **E1-P01** `XL` **The deterministic simulator.** Virtual time, seeded scheduling and ordering, device models for blk, net and gpu, and component substitution.
  ...
  *exit:* **not met, and the reason is now a number rather than a hope.** The workload half reproduces from `(seed, commit)`. The pair is not yet over one component set, because nothing spawns a second place: that is `E1-B05`'s and `E1-B08`'s, and until it closes no artefact here claims the boot ran what the simulator ran.
...
- [>] **E1-P06** `M` Driver chaos: kill each driver component at random under sustained load.
  ...
  *exit:* **half met, and the missing half is a declared quantity rather than a silence.** The blast-radius claim is gating. But what is killed is a *modelled* occupant of a real place, not a scheduled ring-3 driver component, because `kernel/src/blk.rs` still calls `Driver::execute`. `CHAOS_GAP` in `xtask` checks on every run that the reason is still true and goes red the day it stops being — the same discipline RFC 0036 applied to the join. Closing it is `E1-B08`'s two remaining moves.
  *needs:* E1-B05
