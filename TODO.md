...
- [>] **E2-B04** `L` The configuration evaluator: one expression to one generation root.
  ...
  *exit:* partly met, and it cannot be more than that from here. **One machine twice is not two machines.** Both containers share one checkout path and one host, so the path-dependence class of divergence is untested and `--remap-path-prefix` is still `E2-P06`'s first deliverable. The exit's own words name `E2-P06` as the checker, `E2-P06` does not exist, and no command in this slice can close it — which is why this line is `[>]`. No `needs:` on `E2-P06` is added, because `E2-P06` waits on this line and a cycle is a bug in this file rather than a shape to support.
