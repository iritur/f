...
- [x] **E2-D02** `M` Mutable extents: the design for the workload content addressing is worst at, written as a second object kind rather than as a unification.
  ...
  **What it does not do is resolve the complication**, and RFC 0061 has since made its bad class smaller: periodic content below the target size is served by the chunked kind under the new acceptance rule, so what survives is preallocation and long zero runs. That is still what a freshly-provisioned image and a preallocated database file are made of, so the decision stands — but the class got smaller, 0061 says so rather than leaving it to be noticed, and if `E2-B09` measures the survivor too small to justify two object kinds that is a reversal condition for this entry.
