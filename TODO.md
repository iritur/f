...
- [>] **E2-P02** `M` Property tests for chunking and deduplication.
  ...
  **RFC 0061 is implemented exactly as decided, and its own confirming run does not confirm it.** Acceptance is now a predicate over a window — one `MASK_BITS` = 16 mask drawn under the label `f-blob mask v2`, a candidate accepted when no candidate occurs in the preceding `CHUNK_MIN_BYTES`, the forced cut at `CHUNK_MAX_BYTES` the only surviving reference to the previous boundary. Properties 1, 2, 3 and 5 hold. Property 4 holds against the **published** bound on 32 of 32 pairs, zero violations, and the tighter flat clause is met *exactly* on 12 of the 13 pairs where it applies. What fails is the requirement the entry's *Consequences* state for its confirming run: that the flat clause and not the starved clause carry every periodic and concatenated pair. It carries 8 of those 16 — periodic seeds 2, 3, 4, 6, 8 and concatenated seeds 4, 5, 7 pass only on the starved allowance. That is RFC 0061's first reversal condition, fired.
