...
pub fn rollback() -> Result<(), String> {
    ...
    let restored = boot_selecting(&before.root, &before.frame, &menu)?;
    let elsewhere = boot_selecting(&broken.root, &before.frame, &menu)?;
...
///
/// # Why a frame hash is passed as well as a root
///
/// Because `f.root=` is half a statement. RFC 0012 puts `f.frame=` beside it and
/// `kernel/src/measure.rs` refuses a command line carrying one without the
/// other — `HalfADeclaration` — so a boot that named only a generation would go
/// red before it ever reached the selection this function is about, and the
/// error would name the measurement rather than the token. Every boot below
/// declares the *clean* frame, because every boot below is the clean build:
/// what moves across this command is which module the token picks, not which
/// kernel is running.
fn boot_selecting(root: &[u8; 32], frame: &[u8; 32], menu: &[String]) -> Result<Reported, String> {
    let append = generation::tokens(root, frame);
...
fn refuses(menu: &[String], frame: &[u8; 32]) -> Result<(), String> {
    let append = generation::tokens(&[0; 32], frame);
    let (code, log) = crate::boot_carrying(&append, menu, &[])?;
    if code == Some(OK) {
        return Err(
...
fn tampered(dir: &Path, before: &Packed) -> Result<u16, String> {
    ...
    let append = generation::tokens(&before.root, &before.frame);
    let (code, log) = crate::boot_carrying(&append, &menu, &[])?;
    if code != Some(OK) {
        return Err(format!(
