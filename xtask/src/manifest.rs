...
///
/// Three since RFC 0065, which added the required `[[state]]` array. A schema-2
/// manifest declares no state tree, and a supervisor reading that silence as
/// *this component has nothing to say about itself* is the tolerance RFC 0013
/// was written against — so it is refused and edited, not defaulted.
...
pub fn compile(rel: &str, text: &str, image: &[u8]) -> Result<Vec<u8>, Vec<String>> {
    ...
    put8(&mut out, 101, narrow8(nodes.len()));
...
";
    ...
    fn a_declared_tree_reaches_the_record_with_its_parents_resolved() {
        let bytes = compile("user/example/manifest.toml", SOUND, b"image")
            .unwrap_or_else(|f| panic!("{}", f.join("\n")));
        assert_eq!(bytes[101], 2, "the node count is not where the record carries it");
