...
| `E1-P10` | *“ring submit under load”* — one of its four claims | `runner-class-A` | Nothing yet for this one; the workload is `E0-B12`'s and refuses to record here. **The other three are counts and are deliberately not on this page** — doorbells, copies and kernel entries per operation are identical on any host, and `kernel/src/blk.rs` already asserts `copies == 0` with a provocation behind it. | The machine, for the time. The three counts need registering, which is work. |
