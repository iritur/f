...
| An over-subscribed reservation is admitted | L3, P | `abi/proofs` — `admitting_an_arbitrary_demand` states over every demand and every machine shape up to eight cores that a grant admits no period or slack the frame's clock cannot observe, no memory outside the pool or the grain, and no core the frame keeps; `two_grants_never_share_a_core` is the whole-core rule across two reservations and is what `mutate-overlapping-grant` has to break; those two run again at sixty-four cores. `cargo xtask prove` on the nightly; `cargo xtask lint-proofs` on every verify says the harnesses still compile against the crate. And P: `cargo xtask admission`'s over-subscribed arm must be refused, every verify | nightly, and every verify for the boot | **catches** — within a bound on the machine's size and a history of two grants, both stated in the crate |
