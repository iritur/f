...
| 0071 | `E1-B16` | The shared half of three supervisors is one module, and `Reported` is not in it | draft; the measurement is in the entry and the diff is `E1-B16` — the number is allocated here first because six separate plans for separate work each computed 0071 as next free, which is `docs/postmortem/0001`'s first failure waiting to happen again |
