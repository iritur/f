...
| 0084 | `E3-B01b`, `E3-B01d`, `E3-B04c`, `E3-B07a` | An exit narrowed by measurement is a reversal, and four of E3's are narrowed | accepted; the first RFC here about the *spec* rather than the code. An exit is the sentence a task is accepted on, so narrowing one is a reversal — a spec's description may be edited freely, its `exit:` may not — and the narrowing is written into the spec with this number beside it, because `intent/0012`'s plan is a paste-ready handoff and a module comment that disagrees with a spec line is a false line waiting for a paste. Four narrowings: `Kind` is not closed *the way `Role` already is* and neither is `Role`; a commit is atomic *over a ring whose ordering pair holds*, because a stale slot decodes as the previous frame and closing that is a per-entry sequence in `abi/`; a prediction error is a maximum over a named corpus rather than a bound over a domain; and an effect's two words are unrepresentable-apart in the type rather than refused at a wire boundary that does not exist. **The RFC also records the failure it caused**: within a day, two agents cited it for narrowings it did not contain, because a citation is cheaper to write than a decision — a rule requiring one needs somebody who checks them |
| 0085 | `E3-B03a` | A decide task's exit may not require its implementation, and `E3-B03a`'s did | accepted; an `S`-sized *Decide* task whose exit fired a conditional on its own answer — decide *imported*, and the exit then demands that `third_party/` carry the entry, which is an import, a manifest and a `shape` protocol that no amount of deciding produces. The decision and the evidence for it were separable all along and are now separated: the entry, the ring it is reached over and `"third_party"` in the root `exclude` move to the import task RFC 0082 writes out. Generalises the shape `E3-B04d` showed from the other side — there a `needs:` was satisfiable while the exit was not — so an exit that outruns its own task is a class rather than an incident |
